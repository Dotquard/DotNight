...
exports.run = async (client, message, args) => {
 ...
 if(!['782556468380696616'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
let tag = "◈"
const kayıtlı = message.guild.roles.cache.find(r => r.id === '782557117025353761')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '782561691610251266')
...
.setFooter(`DotNight | Kayıt`)
.setColor('GREEN')
client.channels.cache.get('782375102423105566').send(embed)
